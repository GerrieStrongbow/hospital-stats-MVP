...
function renderPatientListItemHTML(record) {
    ...
    const syncColor = record.isSynced ? 'var(--accent-color)' : 'var(--warning-color)';
    const recordDate = new Date(record.date || record.created_at).toLocaleDateString();
    
    return `
        <div class="list-item" onclick="viewPatient('${record.id || record.local_id}', '${record.source}')">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <div style="display: flex; align-items: center; gap: 8px;">
                    <span style="font-size: 12px; color: ${syncColor};">
                        ${syncIcon} ${record.isSynced ? 'Synced' : 'Pending'}
                    </span>
                </div>
            </div>
        </div>
